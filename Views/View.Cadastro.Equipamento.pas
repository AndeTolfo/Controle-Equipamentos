...
uses
  ...
  Vcl.Buttons, DATAMODULE, Data.DB, View.Pesquisas,DateUtils;
  ...
  begin
    ...
    begin
      ...
      // Limpa os campos do formulário após excluir o registro
          ClearControls([edtCod, edtNome, edtLoja, edtPedido, edtComprador, edtValor, cbTipoCompra, dtCompra, edtQtd, edtTotal]);
    end;
  end
  else
  begin
...
begin
  ...
  try
    ...
    qtdCompra := StrToFloat(edtQtd.Text);
    ...
    begin
      ...
      DM.WriteCompras.SQL.Clear;
      DM.WriteCompras.SQL.Add('INSERT INTO COMPRAS(DATA_COMPRA, NOME_ITEM, LOJA_COMPRA, N_PEDIDO, NOME_COMPRADOR, VALOR_ITEM, ORIGEM_COMPRA,QTD_Comprada,TOTAL)');
      DM.WriteCompras.SQL.Add('VALUES (:pDT, :pNomeItem, :pLoja, :pPedido, :pNomeComprador, :pValor, :pOrigemCompra, :pQtdCompra, :pTotal)');
    end
    else
    begin
      ...
      DM.WriteCompras.SQL.Add('NOME_COMPRADOR = :pNomeComprador, VALOR_ITEM = :pValor, ORIGEM_COMPRA = :pOrigemCompra, Qtd_Comprada = :pQtdCompra, Total = :pTotal');
    ...
    DM.WriteCompras.ParamByName('pQtdCompra').AsFloat := qtdCompra;
    DM.WriteCompras.ParamByName('pTotal').AsFloat := Total;
    DM.WriteCompras.ExecSQL;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        edtQtd.Text := FloatToStr(DM.ReadCompras.FieldByName('qtd_comprada').AsFloat);
        edtTotal.Text := FloatToStr(DM.ReadCompras.FieldByName('total').AsFloat);

        // Define o valor do ComboBox cbTipoCompra com base na coluna ORIGEM_COMPRA do banco de dados
        if DM.ReadCompras.FieldByName('ORIGEM_COMPRA').AsString = 'I' then
          cbTipoCompra.ItemIndex := 0
        else if DM.ReadCompras.FieldByName('ORIGEM_COMPRA').AsString = 'P' then
...
procedure TfrmCadastroCompras.edtTotalChange(Sender: TObject);
 var conta : double;
begin
 if (edtTotal.Text <> '') and (edtQtd.Text <> '') then
 begin
      if (strtofloat(edtTotal.Text) > 0) and (StrToFloat(edtQtd.Text) > 0)then
  begin
    conta := strtofloat(edtTotal.Text) / StrToFloat(edtQtd.Text);
    edtValor.Text := FloatToStr(conta);
  end;
 end
end;
