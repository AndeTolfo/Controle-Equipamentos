...
begin
 ...
 cdsDados.FieldDefs.Clear;
  ...
  cdsDados.FieldByName('COD_EQP').DisplayLabel := 'Código';
  cdsDados.FieldByName('DATA_CADASTRO').DisplayLabel := 'Data de Cadastro';
  cdsDados.FieldByName('DATA_ATUALIZACAO').DisplayLabel := 'Data de Atualização';
  cdsDados.FieldByName('NOME_MAQUINA').DisplayLabel := 'Nome da Máquina';
  ...
  cdsDados.FieldByName('PLACA_MAE').DisplayLabel := 'Placa Mãe';
  cdsDados.FieldByName('HD_SSD').DisplayLabel := 'HD/SSD';
  cdsDados.FieldByName('WINDOWS').DisplayLabel := 'Windows';
  cdsDados.FieldByName('LICENSA').DisplayLabel := 'Licença';
...
begin
  // Verificar se o CDS está aberto antes de carregar os dados
  ...
  dm.ReadEquipamentos.DisableControls; // Desativar a atualização visual temporariamente
  try
    ...
    dm.ReadEquipamentos.EnableControls; // Reativar a atualização visual
  end;
end;
...
begin
  ...
  DM.ReadEquipamentos.Close; // Fechar a conexão com a base de dados após o carregamento
end;
